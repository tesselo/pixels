...
class LandsatBandOption(str, Enum):
    b1 = ("B1",)
    b2 = ("B2",)
    b3 = ("B3",)
    b4 = ("B4",)
...
class PixelsConfigValidator(PixelsBaseValidator, extra=Extra.forbid):
    ...
    def check_composite_method(cls, v, values):
        ...
        elif v == CompositeMethodOption.qa_pixel and values["platforms"] != [
            PlatformOption.landsat_8
        ]:
            raise ValueError(
                f"{v} composite method is only allowed for {PlatformOption.landsat_8.value}"
            )
        elif v in (CompositeMethodOption.scl, CompositeMethodOption.full) and values[
            "platforms"
        ] != [PlatformOption.sentinel_2]:
            raise ValueError(
                f"{v} composite method is only allowed for {PlatformOption.sentinel_2.value}"
            )
        return v
