...
def pixels(data=None):
    ...
    if config["delay"] and not config["tag"]:
        ...
        config["tag"] = utils.generate_unique_key(
            config["format"],
            ts_tag=config.get("base_path", ""),
            ts_tag_is_main_key="base_path" in config,
        )
        print(config["tag"])
        url = "{}async/{}?key={}".format(
            request.host_url, config["tag"], request.args["key"]
        )
        pixels_task(config)
        return jsonify(
            message="Getting pixels asynchronously. Your files will be ready at the link below soon.",
            url=url,
        )
...
def asyncresult(tag, file, basetag=None):
    ...
    try:
        ...
        try:
            ...
            return jsonify(
                message="Async collection falied.",
                data=json.loads(failed["Body"].read()),
            )
...
def tiles(z, x, y, platform="s2"):
    ...
    if platform.lower() == "s2":
        ...
        raise PixelsFailed(
            'Platform "{}" not recognized, should be S1 or S2.'.format(platform)
        )
...
def timeseries():
    ...
    for here_start, here_end in utils.timeseries_steps(
        ...
        logger.info(
            "Getting timeseries data from {} to {}.".format(here_start, here_end)
        )
        ...
        config["tag"] = utils.generate_unique_key(
            config["format"], config["timeseries_tag"]
        )
        url = "{}async/{}?key={}".format(
            request.host_url, config["tag"], request.args["key"]
        )
    ...
    timeseries_url = "{}timeseries/{}/data.zip?key={}".format(
        request.host_url, config["timeseries_tag"], request.args["key"]
    )
...
def timeseries_result(tag):
    ...
    results = [
        dat["Key"]
        for dat in results["Contents"]
        if not dat["Key"].endswith("/ts_steps.json")
    ]
    ...
    if done_count != timesteps_count:
        return jsonify(
            message="Pixel collection not finished yet.".format(
                done_count, timesteps_count
            ),
    ...
    with zipfile.ZipFile(fl.name, "w") as zf:
        for step in steps:
            ...
            step_name = "{}_{}_{}".format(
                step["start"], step["end"], step_key.split("/")[-1]
            )
            # Add file to zip.
...
def composite(projectid, z, x, y, frmt="png"):
        ...
        if formula:
            if frmt == "tif":
                ...
                creation_args = {
                    ...
                    "crs": "epsg:3857",
                    "transform": Affine(
                        scale, 0.0, tbounds[0], 0.0, -scale, tbounds[3]
                    ),
                }
            ...
            pixpix = numpy.array(
                [
                    numpy.clip(data[0], 0, const.SENTINEL_2_RGB_CLIPPER).T
                    * 255
                    / const.SENTINEL_2_RGB_CLIPPER,
                    numpy.clip(data[1], 0, const.SENTINEL_2_RGB_CLIPPER).T
                    * 255
                    / const.SENTINEL_2_RGB_CLIPPER,
                    numpy.clip(data[2], 0, const.SENTINEL_2_RGB_CLIPPER).T
                    * 255
                    / const.SENTINEL_2_RGB_CLIPPER,
                    data[3],
                ],
    ...
    return send_file(
        output, mimetype="image/{}".format("png" if frmt == "png" else "tiff")
    )
