...
job['containerOverrides']['command'] = ['pyramid.py']
job['containerOverrides']['memory'] = 1024 * 2
job['containerOverrides']['vcpus'] = 1
job['arrayProperties'] = {'size': batch_array_size}
collect_train_result = batch.submit_job(**job)

{x['jobName']: x['status'] for x in batch.describe_jobs(jobs=[collect_train_result['jobId']])['jobs']}
