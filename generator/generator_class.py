...
# Defining cloud mask to apply
def cloud_filter(X):
    mask = cloud_or_snow_mask(X[:, 8], X[:, 7], X[:, 6], X[:, 2], X[:, 1], X[:, 0], X[:, 9])
...
class DataGenerator_NPZ(keras.utils.Sequence):
    ...
    def __init__(self, path_work, split=1, train=True, train_split=[], shuffle=False, mode='PIXEL', num_time=12, seed=None, , upsampling=False):
    ...
    def on_epoch_end(self):
        ...
        '''
        indexes = np.random.choice(self.DataBaseSize, self.steps_per_epoch, replace=False)
