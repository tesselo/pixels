...
data = json.load(
    open("/media/tam/rhino/work/projects/tesselo/projects/neer/neer_parcels.json")
)
...
for parcel in data_links.keys():
    target = (
        "/media/tam/rhino/work/projects/tesselo/projects/neer/data/" + parcel + "/*"
    )
...
numpy.savetxt(
    "/media/tam/rhino/work/projects/tesselo/projects/neer/results/ndvi.csv",
    table,
    header="parcel,start,end,mean,max,min,std",
    fmt="%s",
    comments="",
    delimiter=",",
)
...
df[(df["species"] == "Eucalyptus") * (df["max"] > 0.2)].dropna().groupby("parcel")[
    "rolling"
].plot()
plt.show()
