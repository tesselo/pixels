...
for i, (x, y, z) in enumerate(burn(rep['features'], 11)):
    ...
    cloud_mask = cloud_or_snow_mask(X[:, :, 8], X[:, :, 7], X[:, :, 6], X[:, :, 2], X[:, :, 1], X[:, :, 0], X[:, :, 9], cloud_only=True)
    X[cloud_mask] = 0
    ...
    with rasterio.open('/home/tam/Desktop/pixels_test/nonseq_zxy/prediction_{}_{}_{}.tif'.format(z, x, y), 'w', **creation_args) as dst:
