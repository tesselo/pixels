...
train_or_predict = (
    "predict" if os.environ.get("PREDICT", "false").lower() == "true" else "train"
)
# Fetch config.
s3 = boto3.client("s3")
...
for page in data_files:
    for npz in page["Contents"]:
        ...
        if train_or_predict == "train":
            training_y.append(
                numpy.ones(data["data"].shape[0])
                * class_lookup[data["class_name"].item()]
            )

# Stack data and add to numpy savez ready dict.
to_save = {"X": numpy.vstack(training_x)}
if train_or_predict == "train":
